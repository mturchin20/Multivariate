...
HOMA_B <- cbind(HOMA_B, apply(as.matrix(HOMA_B$pvalue), 2, GetZScore))
HOMA_IR <- cbind(HOMA_IR, apply(as.matrix(HOMA_IR$pvalue), 2, GetZScore))
FastIns <- cbind(FastIns, apply(as.matrix(FastIns$pvalue), 2, GetZScore))
FastGlu <- cbind(FastGlu, apply(as.matrix(FastGlu$pvalue), 2, GetZScore))

HOMA_B.colnames <- colnames(HOMA_B)
HOMA_B.colnames[9] <- "GC.Zscore"
colnames(HOMA_B) <- HOMA_B.colnames
HOMA_IR.colnames <- colnames(HOMA_IR)
HOMA_IR.colnames[9] <- "GC.Zscore"
colnames(HOMA_IR) <- HOMA_IR.colnames
FastIns.colnames <- colnames(FastIns)
FastIns.colnames[9] <- "GC.Zscore"
colnames(FastIns) <- FastIns.colnames
FastGlu.colnames <- colnames(FastGlu)
FastGlu.colnames[9] <- "GC.Zscore"
...
dt2 = data.table(rs=HOMA_IR$snp,Z.HOMA_IR=HOMA_IR$GC.Zscore,key="rs")
dt3 = merge(dt1,dt2)
rm(dt2)
rm(dt1)
dt1 = data.table(rs=FastIns$snp,Z.FastIns=FastIns$GC.Zscore,key="rs")
dt2 = merge(dt3,dt1)
rm(dt1)
rm(dt3)
...
write.table(file="MAGIC2010.dtsignif.vs1.txt",dtsignif,sep=",",row.names=F,quote=F)
write.table(file="MAGIC2010.dtsignif.rs.vs1.txt",dtsignif$rs,sep=",",row.names=F,quote=F)
write.table(file="MAGIC2010.dtlesssignif.vs1.txt",dtlesssignif,sep=",",row.names=F,quote=F)
write.table(file="MAGIC2010.dtlesssignif.rs.vs1.txt",dtlesssignif$rs,sep=",",row.names=F,quote=F)
#write.table(file="MAGIC2010.dtlesssignif.rs.vs1.txt",paste(dtlesssignif$rs,"[[:space:]]",sep=""),row.names=F,quote=F)

write.table(file="MAGIC2010.RSS0.vs1.txt",RSS0,row.names=F,sep=",",quote=F)
