...
#> head(dt3)
#rs     Z.SBP      Z.DBP
#1:       rs10 1.1388274 1.53991001
#2:  rs1000000 1.3658297 1.46240099
#3: rs10000010 0.1693811 0.83763902
#4: rs10000012 0.7245045 0.06563208
#5: rs10000013 1.6038426 1.63559284
#6: rs10000017 0.2132554 0.15217732
#~~~

SBP.maxZ <- max(dt3$Z.SBP[!is.infinite(dt3$Z.SBP) & !is.na(dt3$Z.SBP)])
DBP.maxZ <- max(dt3$Z.DBP[!is.infinite(dt3$Z.DBP) & !is.na(dt3$Z.DBP)])
maxZ <- max(c(SBP.maxZ, DBP.maxZ))
...
dim(dt3)
dt3 <- dt3[!is.na(dt3$Z.SBP) & !is.na(dt3$Z.DBP),]
dim(dt3)
#~~~
#> dim(dt3)
#[1] 2687669       3
...
#~~~
attach(dt3)
nullset = (abs(dt3$Z.DBP)<2) & (abs(dt3$Z.SBP)<2) #extract null Z values
Z = cbind(Z.DBP,Z.SBP)
...
#> RSS0
#      Z.DBP     Z.SBP
#Z.DBP 1.0000000 0.3652243
#Z.SBP 0.3652243 1.0000000
#~~~

mvstat =  rowSums(Z * (Z %*% RSS0inv)) # comptues Z RSS0^-1 Z'
dt3$mvstat = mvstat
statchi = -log10(exp(1))*pchisq(mvstat,df=2,log.p=TRUE,lower.tail=FALSE)
dt3$mvp = statchi
...
#> quantile(dt3$mvp)
#0%          25%          50%          75%         100%
#8.350331e-08 7.750221e-02 2.066881e-01 4.703999e-01 1.555176e+01
#> quantile(dt3$unip)
#0%          25%          50%          75%         100%
#2.089423e-04 2.446690e-01 4.518190e-01 7.839706e-01 1.378423e+01
#~~~

#From using all 4 pheno
#~~~
#> dim(dtsignif)
#[1] 495   8
#> dim(dtlesssignif)
...
write.table(file="ICBP2011.OnlySBPDBP.dtsignif.vs1.txt",dtsignif,sep=",",row.names=F,quote=F)
write.table(file="ICBP2011.OnlySBPDBP.dtsignif.rs.vs1.txt",dtsignif$rs,sep=",",row.names=F,quote=F)
write.table(file="ICBP2011.OnlySBPDBP.dtlesssignif.vs1.txt",dtlesssignif,sep=",",row.names=F,quote=F)
write.table(file="ICBP2011.OnlySBPDBP.dtlesssignif.rs.vs1.txt",dtlesssignif$rs,sep=",",row.names=F,quote=F)
#write.table(file="ICBP2011.OnlySBPDBP.dtlesssignif.rs.vs1.txt",paste(dtlesssignif$rs,"[[:space:]]",sep=""),row.names=F,quote=F)

write.table(file="ICBP2011.OnlySBPDBP.RSS0.vs1.txt",RSS0,row.names=F,sep=",",quote=F)
