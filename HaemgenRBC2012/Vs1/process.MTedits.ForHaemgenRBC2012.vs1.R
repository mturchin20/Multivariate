...
RBC <- cbind(RBC, apply(as.matrix(RBC$Pval), 2, GetZScore))
MCV <- cbind(MCV, apply(as.matrix(MCV$Pval), 2, GetZScore))
PCV <- cbind(PCV, apply(as.matrix(PCV$Pval), 2, GetZScore))
MCH <- cbind(MCH, apply(as.matrix(MCH$Pval), 2, GetZScore))
Hb <- cbind(Hb, apply(as.matrix(Hb$Pval), 2, GetZScore))
MCHC <- cbind(MCHC, apply(as.matrix(MCHC$Pval), 2, GetZScore))

RBC.colnames <- colnames(RBC)
RBC.colnames[11] <- "GC.Zscore"
colnames(RBC) <- RBC.colnames
MCV.colnames <- colnames(MCV)
MCV.colnames[11] <- "GC.Zscore"
colnames(MCV) <- MCV.colnames
PCV.colnames <- colnames(PCV)
PCV.colnames[11] <- "GC.Zscore"
colnames(PCV) <- PCV.colnames
MCH.colnames <- colnames(MCH)
MCH.colnames[11] <- "GC.Zscore"
colnames(MCH) <- MCH.colnames
Hb.colnames <- colnames(Hb)
Hb.colnames[11] <- "GC.Zscore"
colnames(Hb) <- Hb.colnames
MCHC.colnames <- colnames(MCHC)
MCHC.colnames[11] <- "GC.Zscore"
...
write.table(file="HaemgenRBC2012.dtsignif.vs1.txt",dtsignif,sep=",",row.names=F,quote=F)
write.table(file="HaemgenRBC2012.dtsignif.rs.vs1.txt",dtsignif$rs,sep=",",row.names=F,quote=F)
write.table(file="HaemgenRBC2012.dtlesssignif.vs1.txt",dtlesssignif,sep=",",row.names=F,quote=F)
write.table(file="HaemgenRBC2012.dtlesssignif.rs.vs1.txt",dtlesssignif$rs,sep=",",row.names=F,quote=F)
#write.table(file="HaemgenRBC2012.dtlesssignif.rs.vs1.txt",paste(dtlesssignif$rs,"[[:space:]]",sep=""),row.names=F,quote=F)

write.table(file="HaemgenRBC2012.RSS0.vs1.txt",RSS0,row.names=F,sep=",",quote=F)
